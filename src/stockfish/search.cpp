...
namespace {
  ...
  Value search(Position& pos, TranspositionTable & TT, Stack* ss, Value alpha, Value beta, const Depth depth, bool cutNode);

  template <NodeType NT>
  Value qsearch(Position& pos, TranspositionTable & TT, Stack* ss, Value alpha, Value beta, Depth depth = DEPTH_ZERO);
...
void MainThread::search() {
  ...
  if (rootMoves.empty())
  {
      ...
      sync_cout << "short_info depth 0 score "
         ...
         && !(Limits.depth && mainThread && rootDepth / ONE_PLY > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          while (true)
          {
              ...
              bestValue = ::search<PV>(rootPos, EvalTT, ss, alpha, beta, adjustedDepth, false);
          ...
          reporting::set_bestvalue(bestValue,rootDepth);
...
lczero::optional<CompareableMove> get_bestmove_from_tt(Position & pos){
   ...
   if (ttHit)
   {
       ...
       if (MoveList<LEGAL>(pos).contains(m)){
           CompareableMove cm(UCI::move(m,pos.is_chess960()));
           //std::cout << "found best move\n";
           return lczero::optional<CompareableMove>(cm);
       }
   }
...
void MCTSThread::search(){
   ...
   const int ct = int(Options["Contempt"]) * PawnValueEg / 100; // From centipawns
   Color us = rootPos.side_to_move();
   contempt = (us == WHITE ?  make_score(ct, ct / 2)
                           : -make_score(ct, ct / 2));

    while(!Threads.stop){
        TimeHeapReturn calc_pos_data = reporting::pop_calc_position();
        if(Threads.stop){//check again for stopping threads because pop_calc_position may take awhile
            break;
        }

        int val_depth = reporting::get_bestvalue_depth();
        if(val_depth <= 0){
            continue;
        }
        CompareableMoveList moves_to_pos = calc_pos_data.moves_to_pos;
        Value bestval = reporting::get_bestvalue();
        if(bestval >= VALUE_MATE_IN_MAX_PLY){
        ...
        if (val_depth >= 5 * ONE_PLY){
          // Adjust contempt based on root move's previousScore (dynamic contempt)
          int dct = ct + 88 * bestval / (abs(bestval) + 200);
        ...
        int move_idx = 0;
        for(CompareableMove cm : moves_to_pos){
            std::string movestr = cm.to_string();
            Move move = UCI::to_move(calc_pos,movestr);
            if(!calc_pos.legal(move)){
                throw std::runtime_error("illegal move in stored movelist");
            }
            calc_pos.do_move(move,states[move_idx]);
...
namespace {
  ...
  Value search(Position& pos, TranspositionTable & TT, Stack* ss, Value alpha, Value beta, const Depth depth, bool cutNode) {
    ...
    if (depth < ONE_PLY)
        return qsearch<NT>(pos, TT, ss, alpha, beta);
    ...
    if (!rootNode)
    {
        ...
        if (   Threads.stop.load(std::memory_order_relaxed)
            || pos.is_draw(ss->ply)
            || ss->ply >= MAX_PLY){
            Value val = (ss->ply >= MAX_PLY && !inCheck) ? evaluate(pos)
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Value nullValue = -search<NonPV>(pos, TT, ss+1, -beta, -beta+1, depth-R, !cutNode);
        ...
        if (nullValue >= beta)
        {
            ...
            Value v = search<NonPV>(pos, TT, ss, beta-1, beta, depth-R, false);
        ...
        &&  abs(beta) < VALUE_MATE_IN_MAX_PLY)
    {
        ...
        while (  (move = mp.next_move()) != MOVE_NONE
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                value = -qsearch<NonPV>(pos, TT, ss+1, -rbeta, -rbeta+1);

                // If the qsearch held perform the regular search
                if (value >= rbeta)
                    value = -search<NonPV>(pos, TT, ss+1, -rbeta, -rbeta+1, depth - 4 * ONE_PLY, !cutNode);
    ...
    if (    depth >= 8 * ONE_PLY
        && !ttMove)
    {
        search<NT>(pos, TT, ss, alpha, beta, depth - 7 * ONE_PLY, cutNode);
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
      ...
      if (rootNode && thisThread == Threads.main() && Time.elapsed() > 3000)
          sync_cout << "short_info depth " << depth / ONE_PLY
          ...
          &&  pos.legal(move))
      {
          ...
          value = search<NonPV>(pos, TT, ss, rBeta - 1, rBeta, depth / 2, cutNode);
      ...
      if (PvNode && (moveCount == 1 || (value > alpha && (rootNode || value < beta))))
      {
          ...
          value = -search<PV>(pos, TT, ss+1, -beta, -alpha, newDepth, false);
      }
  ...
  template <NodeType NT>
  Value qsearch(Position& pos, TranspositionTable & TT, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (   pos.is_draw(ss->ply)
        || ss->ply >= MAX_PLY){
            Value val = (ss->ply >= MAX_PLY && !inCheck) ? evaluate(pos) : VALUE_DRAW;
        return val;
    }
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
      ...
      pos.do_move(move, st, givesCheck);
      value = -qsearch<NT>(pos, TT, ss+1, -beta, -alpha, depth - ONE_PLY);
      pos.undo_move(move);
...
string UCI::pv(const Position& pos, Depth depth, Value alpha, Value beta) {
  ...
  for (size_t i = 0; i < multiPV; ++i)
  {
      ...
      auto entry = reporting::get_ab_entry(pos.comp_pos());
